...
$txt['maintain_cache'] = 'Очистить кэш SMF';
...
$txt['body_checking_introduction'] = 'Эта функция преобразует поле, содержащее тексты сообщений в таблице базы данных, в тип TEXT (сейчас оно MEDIUMTEXT). Это позволит уменьшить место, занимаемое сообщениями в базе данных (на 1 байт для каждого сообщения). Если какое-то сообщение, хранящееся в базе данных, будет длиннее 65535 символов — оно будет обрезано и часть текста будет потеряна..';
...
$txt['convert_to_text'] = 'Нет сообщений длиннее 65535 символов. Можно безопасно провести преобразование, без потери данных.';
...
$txt['move_type_sticky'] = 'Закрепленные темы';
...
$txt['plain_text'] = 'Лучший способ создать резервную копию — использовать обычный текстовый файл, сжатая копия не может считаться полностью надежной.';
