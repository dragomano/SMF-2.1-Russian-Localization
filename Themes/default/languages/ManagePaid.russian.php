...
$txt['paid_email_to'] = 'Имейл для сообщений';
...
SMF использует <strong>PayPal</strong> как платежный метод по умолчанию.<br>
	&bull; Нет необходимости включать IPN в аккаунте PayPal; если вы сделаете это — система подписок форума будет получать уведомления обо всех платежах приходящих на ваш аккаунт и это будет вызывать сообщения об ошибках для платежей, которые не связаны с подписками форума.<br>
...
Если вы используете другие платежные шлюзы, необходимо установить URL-адрес уведомлений для каждого из способов оплаты. Для всех способов оплаты этот URL-адрес должен быть задан так:<br><br>
	...
	Для других платежных шлюзов (если они установлены) этот адрес можно задать в панели аккаунта шлюза — обычно это пункты «Return URL», «Callback URL» или «Адрес уведомлений».<br><br><br>';
...
$txt['paid_mod_email_desc'] = 'Обозначения: {NAME} — имя пользователя; {FORUM} — название форума. Тема сообщения должна быть указана в первой строке. Оставьте пустым для отключения уведомлений.';
