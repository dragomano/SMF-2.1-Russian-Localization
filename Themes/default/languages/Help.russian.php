...
Эта надпись будет отображаться под именем.<br><em>Например:</em><br>Я есть Грут<br>Я крут!';
$helptxt['onlineEnable'] = 'Отображение картинки с текущим статусом пользователя.';
$helptxt['todayMod'] = 'Отображение слов «Сегодня» или «Завтра» вместо полной даты.<br><br>
...
$helptxt['samesiteCookies'] = 'Атрибут SameSite позволяет установить, требуется ли вашему сайту получать куки при запросах с разных доменов.<br><br>
	...
	</ul><br><br>.За дополнительной информацией обращайтесь к <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie/SameSite">документации MDN</a>';
...
$helptxt['jquery_source'] = 'Это определит источник, используемый для загрузки библиотеки jQuery. <em>Авто</em> — сначала будет использоваться CDN, а в случае неудачи будет производиться подключение файла из локальной директории. <em>Локальный источник</em> — загрузка из директории форума. <em>CDN</em> — загрузка с серверов Google CDN';
...
$helptxt['time_offset'] = 'Некоторые системы не хранят время в формате UTC. Используя эту функцию, можно указать разницу во времени (в часах) между серверным временем и UTC. Допускаются отрицательные и десятичные значения.';
...
$helptxt['quote_expand'] = 'Эта опция позволяет обрезать большие цитаты и разворачивать их до полного размера. Параметр регулирует минимальную высоту, которую должна иметь цитата для после обрезания. Цитаты, имеющие меньший объём текста, затронуты не будут.';
...
$helptxt['pm_posts_verification'] = 'Эта опция заставит пользователей вводить код, отображаемый на изображении, каждый раз при отправке личного сообщения. Пользователи, имеющие определенное количество сообщений, не будут вводить код &mdash; это должно помочь в борьбе с автоматическими рассылками спама.';
...
$helptxt['smiley_sameall'] = 'Если вы установите флажок «То же изображение для всех наборов», то одно и то же изображение будет добавлено ко всем вашим наборам смайликов. Если вы снимите этот флажок, то для каждого установленного набора смайликов можно будет загрузить другое изображение. Обратите внимание, что для каждого набора смайликов необходимо предоставить изображение.';
...
$helptxt['show_spider_online'] = 'С помощью этого параметра можно выбрать, кому будут видны поисковые боты (их имена, количество) в списке «Сейчас на форуме». Доступные варианты:
	<ul class="normallist">
		<li>
			...
			Как указано выше, только администраторы могут видеть статус поисковых ботов &mdash; всем остальным пользователям боты будут представляться в виде гостей.
		</li>
	</ul>';
...
$helptxt['translatable_fields'] = 'Если вы хотите обеспечить многоязычную поддержку полей пользовательского профиля, вы можете использовать переводимые токены как в полях имени, так и в полях описания.<br>
Всё, что заключено в фигурные скобки {}, будет рассматриваться как токен и будет сопоставлено со строкой $txt, например, {gender} будет заменено значением переменной $txt[\'gender\']<br>
<strong>Важно</strong>: соответствующую строку $txt нужно будет добавить для <strong>ВСЕХ</strong> установленных языков, поместив её, например, в index.language.php, чтобы перевод был доступен везде.';
...
$helptxt['requireAgreement'] = 'Этот параметр рекомендуется включить, чтобы соответствовать правилам <a href="https://ec.europa.eu/commission/priorities/justice-and-fundamental-rights/data-protection/2018-reform-eu-data-protection-rules_en" target="_blank" class="bbc_link" rel="noopener">GDPR</a>.';
// The GDPR page of the EU exists in several languages; change the language code at the end of the url
$helptxt['requirePolicyAgreement'] = 'Этот параметр рекомендуется включить, чтобы соответствовать правилам <a href="https://ec.europa.eu/commission/priorities/justice-and-fundamental-rights/data-protection/2018-reform-eu-data-protection-rules_en" target="_blank" class="bbc_link" rel="noopener">GDPR</a>.';
